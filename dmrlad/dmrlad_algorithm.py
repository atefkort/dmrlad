...
class DMRLADAlgorithm:
    ...
    def train(self):
        ...
        if self.num_transitions_initial > 0:
            self._n_env_steps_total += self.rollout_coordinator.collect_initial_replay_data(self.train_tasks, max_samples=self.num_transitions_initial)
        ...
        for epoch in gt.timed_for(range(self.num_epochs), save_itrs=True):
            ...
            self._n_env_steps_total += self.rollout_coordinator.collect_replay_data(data_collection_tasks, self.log_Z, max_samples=self.num_transitions_per_episode)
    ...
    def compute_log_Z(self):
        ...
        for i in range(20):
            start_obs , task_z = self.replay_buffer.sample_start_obs(indices, batch_size=self.utility_batch_size)
            ...
            with torch.no_grad():
                ...
                next_tau, next_tau_hat, next_presum_tau = self.policy_trainer.get_tau(obs, actions, fp=self.policy_trainer.target_fp)
                z1_values = self.policy_trainer.zf1(obs, actions, next_tau_hat)
                z2_values = self.policy_trainer.zf2(obs, actions, next_tau_hat)
                min_qs = torch.min(z1_values, z2_values)
