...
class StackedReplayBuffer:
    def __init__(self, max_replay_buffer_size, time_steps, observation_dim, action_dim, task_indicator_dim, data_usage_reconstruction, data_usage_sac, num_last_samples, permute_samples, encoding_mode, tasks):
    ...
    def add_episode(self, episode):
        ...
        self.task_ids.append(episode['task_id'])
    ...
    def random_start_obs(self, task, batch_size): 
        indices = [i for i, x in enumerate(self.task_ids) if x == task]
        starts = [self._episode_starts[i] for i in indices]
        starts = np.array(starts)
        idx = np.random.choice(starts, batch_size)

        return dict(
            observations=self._observations[idx],
            task_indicators=self._task_indicators[idx]
        )
    ...
    def sample_start_obs(self, indices, batch_size=None):
        ...
        z = []
        for idx in indices:
            batch = self.random_start_obs(idx, batch_size=batch_size) 
            obs.append(batch['observations'])
            z.append(batch['task_indicators'])
        obs = np.array(obs)
        z = np.array(z)
