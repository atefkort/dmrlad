...
class RolloutCoordinator:
    ...
    def relabel_hfr_q(
        ...
        for idx, goal in enumerate(self.train_goals):
            ...
            task_z = self.reinfer_context(data, start_obs[0])
    ...
    def relabel_parallel(self, workers, path, log_Z):
        data = dict(
            observations = path['observations'],
            actions = path['actions'],
            rewards = path['rewards'],
        ...
        q_vals = ray.get([worker.compute_q_vals_from_list.remote(data) for worker in workers])
        ...
        path["rewards"] = rewards.reshape(-1, 1)
        path["terminals"] = terminals.reshape(-1, 1) 
        #path["task_id"] = task_idx   
        self.replay_buffer.add_episode(path)
        self.agent.to(ptu.device)
        self.agent.policy.to(ptu.device)
        self.agent.encoder.to(ptu.device)
        self.zf1.to(ptu.device)
        self.zf2.to(ptu.device)

    def reinfer_context(self, data, start_obs):
...
@ray.remote
class RemoteRelabelWorker(RolloutWorker):
    def __init__(self,
                 env,
                 zf1,
                 zf2,
        ...
        self.stats_dict = replay_buffer_stats_dict
        self.task_list = task_list
    ...
    def compute_q_vals_from_list(self, data):
        q_vals = [] 
        for task, start_ob in zip(self.task_list, self.start_obs):
            result = self.compute_q_vals(data, start_ob, task)
            q_vals.append(result)

        return q_vals

    def compute_q_vals(self, data, start_obs, goal):
        rewards = np.array([self.env.reward(info, goal)[0] for info in data["env_infos"]])
        data["rewards"] = rewards

        task_z = self.infer_task_z(data, start_obs)
        start_obs = ptu.from_numpy(start_obs)           
        task_z = ptu.from_numpy(task_z)
        ...
        q_vals = ptu.get_numpy(torch.mean(min_qs))
        return q_vals

    def infer_task_z(self, data, start_obs):
        self.action_space = self.env.action_space.low.size
        self.obs_space = self.env.observation_space.low.size
        observations = data['observations']
        actions = data['actions']
    ...
    def update_context(self, o, a, r, next_o):
        if self.use_data_normalization and self.stats_dict is not None:
